...
boots_2sCOPE <- function(formula, data_cleaned, dependent_var, independent_P_vars,
                         independent_X_vars, X, cdf, design_matrix1, full_formula,
                         has_intercept) {
  ...
  lm_P_residuals <- matrix(NA, nrow = nrow(P_star1), 
  ...
  for (i in seq_along(independent_P_vars)) {
    ...
    P_star1_reduced <- P_star1[, !(colnames(P_star1) %in% independent_P_vars) | (colnames(P_star1) == p_var), drop = FALSE]
    
    formula_P <- as.formula(paste(p_var, "~ ."))
    lm_P <- lm(formula_P, data = as.data.frame(P_star1_reduced))
    lm_P_residuals[, i] <- residuals(lm_P)
  }
  
  # rename columns of P_star matrix
  colnames(lm_P_residuals) <- paste0(independent_P_vars, "_cop")
  
  # merge columns
  est_matrix <- cbind(full_matrix, lm_P_residuals)
...
CopReg2sCOPE <- function(formula, data, cdf, nboots = 199) {
  ...
  if (length(f1) == 1) {
    ...
    print("Estimation done. Calculating bootstrap standard errors")
    trapped <- pbsapply(1:nboots, 
                        function(i) boots_2sCOPE(formula = formula,
                                                 data_cleaned = data_cleaned,
                                                 dependent_var = dependent_var,
